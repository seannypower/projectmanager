...
export function TaskCard({ task, selected, snoozeOn, onClick, kanban }) {
  ...
  return (
    ...
    >
      {kanban ? (
        <>
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 8 }}>
            ...
            {showSnooze && <Chip bg={snoozeBg} style={{ padding: '2px 8px', fontSize: 10.5 }}>☾ {task.snooze}</Chip>}
          </div>
      ...
      <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6, marginTop: 12 }}>
        ...
        {!kanban && showSnooze && <Chip bg={snoozeBg}>☾ {task.snooze}</Chip>}
      </div>
